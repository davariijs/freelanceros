...
export const emailService = {
  async sendVerificationCode(email: string, code: string): Promise<void> {
    const mailOptions = {
      from: `"FreeOS" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Verify your FreeOS account',
      text: `Welcome to FreeOS. Your 6-digit verification code is: ${code}`,
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e5e5; border-radius: 16px;">
          <h2 style="font-size: 20px; font-weight: 800; color: #171717; margin-bottom: 8px;">Welcome to FreeOS</h2>
  ...
  async sendPasswordResetCode(email: string, code: string): Promise<void> {
    const mailOptions = {
      from: `"FreeOS" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Reset your FreeOS password',
    ...
    const html = `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e5e5; border-radius: 16px;">
        ...
        <div style="border-t: 1px solid #e5e5e5; padding-top: 16px; margin-top: 24px;">
          <p style="font-size: 10px; color: #a3a3a3; margin: 0;">FreeOS Administrative System.</p>
        </div>
      </div>
    `;
